...
apt update
apt upgrade -y
apt install -y wget curl mc net-tools apt-utils apt-transport-https tuned lm-sensors aptitude locales tzdata dialog
...
apt update
apt install -y psmisc pv htop iotop iftop nethogs iptstate apachetop dnstop powertop atop nodejs git-core jpegoptim optipng pngquant gifsicle webp autoconf gcc make libzstd1 libzstd-dev zstd libpcre2-dev libssl-dev linux-headers-generic linux-headers-generic build-essential catdoc poppler-utils poppler-data pkg-config libbrotli-dev libcurl4-openssl-dev lz4 liblz4-1 liblz4-dev sqlite3 libsqlite3-dev libc-ares-dev libc-ares2 liburing2 liburing-dev libnginx-mod-http-headers-more-filter sudo
